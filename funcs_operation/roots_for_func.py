...
def roots_for_func(func, x_min, x_max, eps):
    if x_max <= x_min:
        exit('Недопустимые вводные данные')
    ROOTS = []; x = x_min
    func = streplace(func, 'there')
    ...
    while (x_min <= x <= x_max):
        ...
        if (eval(func) * eval(func.replace('x','x_p'))) / (eval(dfunc) * eval(dfunc.replace('x','x_p'))) <= 0:
            ...
            while (abs(x_0 - x_1) >= 10 ** -(eps + 2) and (x_min <= x_1 <= x_max)):
                x_0 = x_1
                x_1 -= eval(func.replace('x','x_1')) / eval(dfunc.replace('x','x_1'))
            if x_min <= x_1 <= x_max:
                ROOTS.append(sign(round(x_1, 1))*abs(round(x_1, eps)))
        x += x_p - x
    if ROOTS != []:
        print('Для F(x) =', streplace(func, 'back'), 'при x ∈ [' + str(x_min) + ', ' + str(x_max) + '] обнаружены корни:', ROOTS)
    else:
